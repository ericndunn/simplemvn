...
pipeline {
    agent { label 'MASTER' }//def userInput = input(
    //id: 'userInput', message: 'Let\'s promote?', parameters: [
    //[$class: 'TextParameterDefinition', defaultValue: 'uat', description: 'Environment', name: 'env'],
    //[$class: 'TextParameterDefinition', defaultValue: "${TARGET_ENVIRONMENT}", description: 'Target Environment', name: 'target']])
    ...
    stages {
        ...
        stage('Git source files') {
            steps {
                git branch: 'Bespin', credentialsId: '62c93b86-c4ba-483c-a696-8180694ce559', poll: false, url: 'ssh://bitbucket.anthem.com:7999/madt/medicaid-ui.git'
